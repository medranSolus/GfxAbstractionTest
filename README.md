...
In this test there was 3 main architectures run under tests, named **Cache**, **Virtual** and **Proxy**, with 3 classes representing implementations for different graphics APIs.
Additionally Cache and Proxy are also tested used with or without base class **Res** that would represent common resource class. Tests with that variant are marked with prefix.

In Cache and Proxy architectures implementations are hold inside `Impl` union allowing to save space on each possible implementation at cost of always being the size of the biggest implementation object.
...
- **Cache**
  ...
  - Class `Base` holding key to the actual implementation and accessing it's data via `GetData` function,
  - Switching API based on iterating over cache, calling destructor on every implementation on initializing with new one.
- **Virtual**
  - Class `Base` from which eveery implementation is derived from,
...
- **Proxy**
  ...
  - Accessing data via simple function that decides over global variable which API is active,
  - Switching API based on call to `Switch(Type)` method with new implementation passed in.
...
and overall test speed (tests named **G**) of 3 mocked implementations of graphics APIs: `DirectX` (tests named **DX**), `Vulkan` (tests named **VK**) and `OpenGL` (tests named **GL**).
...
According to current data, possibly it is better to follow architecture represented by `Proxy` since it reduces number of times the memory is allocated and slightly speeds up access time
on cost of slightly more space occupied per object. Another pro might be better usage of cache which could speed everything up even more since **Proxy** avoids trashing CPU instruction pipeline
with accessing virtual methods via traversing vtable for each of them.
